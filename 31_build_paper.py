...
\begin{table}[tbp]
\centering
\caption{Diagnosis by observation condition (API tier; local telemetry $n{=}%ABL_EV_LOC_N%$ cells). Classifiers fitted on non-pilot scenarios, evaluated on the pilot draw; description-only is textual ($-$); WLS row uses iterative-estimated state (Sec.~III), LLM-only.}
\label{tab:ablation}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lccccc}
\toprule
