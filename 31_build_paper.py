...
MDEDERIV = ("With paired discordance rate $p_d$, the two-sided MDE at $\\alpha{=}0.05$ and power $0.80$ is "
...
Tool selection is where the deployment tiers differ most---in \emph{style} before \emph{accuracy}. The permissive accepted-set metric is degenerate (always answering power flow achieves it by construction; random choice scores %RANDTOOL\%%), and the API model exercises exactly that strategy: %PFSHARE%\% of its stated tools are power flow. Under the strict-specific metric (Table~\ref{tab:tools}), the local model names a scenario-\emph{required} tool in %MS1%/%MN%--%MS4%/%MN% of cases (flat across configurations) versus %GS1%/%GN%--%GS4%/%GN% for the API model. When the API model states power flow (%PFN% responses), it is actually required in only %PFAPI%\% of cases---the rest are defaults to the universally accepted answer. The local model states power flow rarely (%PFLN% responses, %PFLL%\% of them required); its dominant choices are contingency (%CTN% responses, %CTREQ%\% required---contingency is required precisely on outage and overload scenarios) and equipment queries. The gap therefore reflects the API model's default rate, not superior local tool knowledge; both models remain far from ceiling, and the binary metric cannot distinguish well-placed tools from lucky guesses.

\subsection{RQ3: Closed-Loop Tool Use}
\label{sec:rq3exec}
The one-shot pilot only \emph{names} tools; RQ3 asks whether the agent can \emph{use} them. A closed-loop extension therefore gives the agent tool access inside the loop: it proposes structured calls with arguments (power flow; a \emph{named} branch or generator outage; an N-1 sweep; OPF; voltage/loading/violation queries), each call is validated against the network---referenced elements must exist---and executed by the real pandapower back-end, and the returned measurements enter the conversation. The agent iterates (at most four turns) and then commits to a diagnosis. On %CL_N% pilot scenarios (API tier): diagnosis %CL_DIAG%\%, over %CL_CALLS% tool calls with %CL_ARGVAL%\% argument validity and %CL_EXEC%\% execution success, converging in %CL_TURNS% turns on average---versus %CL_ONESHOT%\% for the one-shot telemetry condition on the same scenarios. The offline execution check remains as plumbing validation: %RQ3_TOTAL% stated-tool pairs across the pilot rounds, %RQ3_EXEC% executable, all succeeding. What remains open is decision quality: whether recommendations drawn from executed results are operationally safe and constraint-relieving is future work.

\subsection{RQ5: Hallucination and the Cost of Local Inference}
Any-tag hallucinated rows are rare for both models under the automated judge: %GHSEQ% (API) and %GEMMA_HALL% (Local) across E1--E4. Judge flags are not expert-annotated ground truth; confidence calibration is unidentifiable at this sample size and will be reported on the powered sweep. Latency is the deployment trade-off: local inference averages %GEMMA_LAT{} per call versus %GLAT{} for the API round-trip---roughly a %LATRATIO$\times$ difference---with zero marginal API cost and no grid data leaving the premises; acceptable for always-on monitoring, but neither model's latency profile suits closed-loop use (Sec.~\ref{sec:limits}).
