...
# Heavy case handling — default ieee14, overridden via --case
import argparse as _argparse
_parser = _argparse.ArgumentParser(add_help=False)
_parser.add_argument("--case", default="ieee14")
_parser.add_argument("--n-per-class", type=int, default=None)
_args, _ = _parser.parse_known_args()
CASE = _args.case  # e.g., case39, case118, ieee14
if CASE == "ieee14":
    SCENARIO_PREFIX = "IEEE14_SCN"
    N_PER_CLASS = 300
    NETWORK_NAME = "ieee14_re"
elif CASE == "case39":
    SCENARIO_PREFIX = "CASE39_SCN"
    N_PER_CLASS = 500
    NETWORK_NAME = "case39_re"
elif CASE == "case118":
    SCENARIO_PREFIX = "CASE118_SCN"
    N_PER_CLASS = 700
    NETWORK_NAME = "case118_re"
else:
    SCENARIO_PREFIX = CASE.upper() + "_SCN"
    N_PER_CLASS = 300
    NETWORK_NAME = CASE + "_re"
if _args.n_per_class is not None:
    N_PER_CLASS = _args.n_per_class

MASTER_SEED = 20260821


# -----------------------------------------------------------------------------
# CORPUS SIZE — overridden by --case logic above; do not reassign here
# -----------------------------------------------------------------------------
...
RE_HASH_FILE = OUTPUT_DIR / f"{CASE}_re_layout_hash.txt"

POINTS_FILE = OUTPUT_DIR / f"{CASE}_operating_points.csv"
FACTORS_FILE = OUTPUT_DIR / f"{CASE}_op_load_factors.csv"
VOLTAGES_FILE = OUTPUT_DIR / f"{CASE}_op_bus_voltages.csv"
LOADING_FILE = OUTPUT_DIR / f"{CASE}_op_branch_loading.csv"

SCENARIOS_CSV = OUTPUT_DIR / f"{CASE}_scenarios.csv"
SCENARIOS_JSONL = OUTPUT_DIR / f"{CASE}_scenarios.jsonl"
POST_VOLTAGES_FILE = OUTPUT_DIR / f"{CASE}_scenario_post_voltages.csv"
POST_LOADING_FILE = OUTPUT_DIR / f"{CASE}_scenario_post_loading.csv"
CLASS_SUMMARY_FILE = OUTPUT_DIR / f"{CASE}_scenario_class_summary.csv"
VALIDATION_FILE = OUTPUT_DIR / f"{CASE}_stage5_validation_summary.csv"
METADATA_OUTPUT_FILE = OUTPUT_DIR / f"{CASE}_stage5_metadata.json"
...
def _cid_series(df, fallback_prefix):
    """Robust cid access — handles pp.to_json dropping custom cols (ext_grid)"""
    if "cid" in df.columns:
        return df["cid"]
    # fallback: generate deterministic cids
    if fallback_prefix == "slack":
        return pd.Series([f"slack_bus_{int(b)+1}" for b in df.bus], index=df.index)
    return pd.Series([f"{fallback_prefix}_{i}" for i in df.index], index=df.index)
...
def apply_injected_event(
    ...
    if mechanism == "load_change":
        ...
        cids = _cid_series(net.load, "load")
        mask = cids.isin(event["targets"]).values
    ...
    if mechanism == "line_outage":
        ...
        cids = _cid_series(net.line, "line")
        mask = cids.isin(event["targets"]).values
    ...
    if mechanism == "renewable_ramp":
        ...
        cids = _cid_series(net.sgen, "sgen")
        mask = cids.isin(event["targets"]).values
    ...
    if mechanism == "avr_setpoint_shift":
        ...
        cids_gen = _cid_series(net.gen, "gen")
        gen_mask = cids_gen.isin(event["targets"]).values

        net.gen.loc[gen_mask, "vm_pu"] = (
            net.gen.loc[gen_mask, "vm_pu"].values + delta
        )

        cids_slack = _cid_series(net.ext_grid, "slack")
        slack_mask = cids_slack.isin(event["targets"]).values
...
def ladder_e6(
    ...
    for percent in (20.0, 30.0, 40.0):
        ...
        # Heavy case fallback: if region has no loads (39/118), use single random load
        if not targets:
            targets = [str(rng.choice(np.asarray(catalogue["load_ids"])))]
            region = "single_bus_fallback"
    ...
    if weak_lines:
        ...
        if not esc_targets:
            esc_targets = [str(rng.choice(np.asarray(catalogue["load_ids"])))]
        ladder.append(
            make_compound_event(
                [
        ...
        # Fallback for large nets with no weak_lines: random line + surge
        rand_line = str(rng.choice(np.asarray(catalogue["line_ids"])))
        ladder.append(
            make_compound_event(
                [
                    make_line_outage_event(rand_line),
                    make_load_event(
                        [str(rng.choice(np.asarray(catalogue["load_ids"])))],
                        30.0,
                        "single_bus",
                    ),
                ],
                "undervoltage_escalation_fallback",
            )
        )
    # Heavy large-net extra aggression: 60% system surge + double outage
    if CASE != "ieee14":
        ladder.append(make_load_event(list(catalogue["load_ids"]), 60.0, "system"))
        ladder.append(make_load_event(list(catalogue["load_ids"]), 80.0, "system"))
        # double outage + 40% surge
        r1 = str(rng.choice(np.asarray(catalogue["line_ids"])))
        r2 = str(rng.choice(np.asarray(catalogue["line_ids"])))
        ladder.append(
            make_compound_event(
                [make_line_outage_event(r1), make_line_outage_event(r2), make_load_event(list(catalogue["load_ids"]), 40.0, "system")],
                "undervoltage_double_outage",
            )
        )
...
def ladder_e8(
    ...
    for cid in corridors:
        ...
        ladder.append(make_line_outage_event(cid))
    # Fallback for large nets where corridors empty
    if not corridors:
        rand_line = str(rng.choice(np.asarray(catalogue["line_ids"])))
        ladder.append(make_line_outage_event(rand_line))
    ...
    if corridors:
        ...
        rand_line2 = str(rng.choice(np.asarray(catalogue["line_ids"])))
        ladder.append(
            make_compound_event(
                [
                    make_line_outage_event(rand_line2),
                    make_load_event(
                        list(catalogue["load_ids"]), 20.0, "system"
                    ),
                ],
                "overload_escalation_fallback",
            )
        )
    if CASE != "ieee14":
        ladder.append(make_load_event(list(catalogue["load_ids"]), 60.0, "system"))
...
def build_catalogue(
    ...
    """
    # Robust cid access (pp.to_json may drop custom cols for ext_grid)
    def get_cids(df, prefix):
        if "cid" in df.columns:
            return list(df["cid"])
        # fallback generate
        if prefix=="slack":
            return [f"slack_bus_{int(b)+1}" for b in df.bus]
        return [f"{prefix}_{i}" for i in df.index]
    return {
        "load_ids": list(net.load["cid"]) if "cid" in net.load.columns else get_cids(net.load,"load"),
        "load_buses": [int(b) for b in net.load["bus_ieee"]] if "bus_ieee" in net.load.columns else [int(b)+1 for b in net.load.bus],
        "line_ids": list(net.line["cid"]) if "cid" in net.line.columns else get_cids(net.line,"line"),
        "trafo_ids": list(net.trafo["cid"]) if "cid" in net.trafo.columns else get_cids(net.trafo,"trafo"),
        "gen_ids": list(net.gen["cid"]) if "cid" in net.gen.columns else get_cids(net.gen,"gen"),
        "slack_ids": list(net.ext_grid["cid"]) if "cid" in net.ext_grid.columns else get_cids(net.ext_grid,"slack"),
        "bus_ids": list(net.bus["cid"]) if "cid" in net.bus.columns else get_cids(net.bus,"bus"),
        "pv_id": str(net.sgen["cid"].iloc[0]) if "cid" in net.sgen.columns else str(net.sgen.index[0]),
        "wind_id": str(net.sgen["cid"].iloc[1]) if "cid" in net.sgen.columns and len(net.sgen)>1 else str(net.sgen.index[1] if len(net.sgen)>1 else "sgen_WIND"),
        "bess_id": str(net.storage["cid"].iloc[0]) if "cid" in net.storage.columns else str(net.storage.index[0]),
    }
        ...
        while produced < N_PER_CLASS:
            ...
            if event_class in OUTCOME_CLASSES:
                ...
                for candidate in ladder[:MAX_SEARCH_ATTEMPTS]:
                    ...
                    # Heavy large-net fix: skip NaN voltages (islanding)
                    if np.isnan(candidate_state["v_min_pu"]) or np.isnan(candidate_state["v_max_pu"]):
                        continue
                ...
                if np.isnan(post_state["v_min_pu"]) or np.isnan(post_state["v_max_pu"]):
                    abandoned[event_class] += 1
                    continue
            ...
            # Use actual storage bus from net for heavy cases (39/118 BESS at different bus)
            try:
                bess_bus = int(net.storage.bus.iloc[0])
            except:
                bess_bus = int(stage3.BESS_CONFIG["bus"])
